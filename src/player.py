"""The player module, containing the Player class."""

import sys
from typing import Optional

from src.entity import Entity
from src.living_entity import LivingEntity, FIGHTING_RANGE
...
class Player(LivingEntity):
    ...
    def _is_in_fighting_range(self, entity: "Entity") -> bool:
        ...
        if Game.GAME is None:
            raise ValueError("Game instance is not available")
    ...
    def _attack_nearby_threats(self) -> None:
        ...
        for entity in Game.GAME.placed_entities:
            if (
                self._is_in_fighting_range(entity)
                and isinstance(entity, Mob)
                and entity.is_aggressive
            ):
                self.attack(entity)
                made_attack = True
        ...
        if not made_attack:
            Game.GAME.add_text(
                f"There were no nearby threats for {self.type} to attack."
            )
