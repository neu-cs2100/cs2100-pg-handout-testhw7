"""Module for Minecraft mobs (mobile entities)"""
...
from src.living_entity import LivingEntity
...
class Behavior(Enum):
    """Behavior options for Minecraft mobs."""
...
class Mob(LivingEntity):
    ...
    def __init__(  # pylint: disable=too-many-arguments disable=too-many-positional-arguments
        ...
        if sound_file_name is None:
            ...
            self.sound = (
                None
                if Mob.minim is None
                else Mob.minim.Sound(f"data/sounds/{sound_file_name}")
            )
    ...
    def is_aggressive(self) -> bool:
        ...
        if self.behavior == Behavior.PASSIVE:
            return False
        if self.behavior in [Behavior.BOSS, Behavior.HOSTILE]:
            return True
        if self.behavior == Behavior.NEUTRAL:
    ...
    def _make_noise(self) -> None:
        ...
        if self.subtitle is not None and Game.GAME is not None:
    ...
    def tick(self) -> None:
        ...
        if self.is_aggressive and Game.GAME:
            from src.living_entity import FIGHTING_RANGE
